...
smt_astt smt_convt::convert_bitcast(const expr2tc &expr)
{
  ...
  if(is_floatbv_type(to_type))
  {
    ...
    {
      log_error("Unions not supported when bitcasting to fp for now");
      expr->dump();
      abort();
    }
  }
  else if(is_bv_type(to_type))
  {
    ...
    {
      log_error("Unions not supported when bitcasting to bv for now");
      expr->dump();
      abort();
    }
  }
  else if(is_struct_type(to_type))
  {
    ...
    if(is_union_type(new_from))
    {
      log_error("Unions not supported when bitcasting to struct for now");
      expr->dump();
      abort();
    }
  }
