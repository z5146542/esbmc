  ...
  unsigned &count)
{
  for(const auto &instruction : goto_program.instructions)
  {
    if(instruction.is_assert())
    {
      ...
      log_status("Claim", std::to_string(count), ":");
      log_status(instruction.location.as_string());
      log_status(description.as_string());
      log_status(from_expr(ns, identifier, instruction.guard));
    }
  }
}
